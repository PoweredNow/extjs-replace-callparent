...
var _babelPluginTransformEs2015Parameters = _interopRequireDefault(require("babel-plugin-transform-es2015-parameters"));
function _interopRequireDefault(e) { return e && e.__esModule ? e : { "default": e }; }
function _default(_ref) {
  var t = _ref.types;
  var transformParametersPlugin = (0, _babelPluginTransformEs2015Parameters["default"])({
    types: t
  });
  function isThisOrMeExpression(node) {
    return t.isThisExpression(node) || t.isIdentifier(node, {
      name: 'me'
    });
  }
  function isCallParentCallee(node) {
    return t.isMemberExpression(node) && isThisOrMeExpression(node.object) && t.isIdentifier(node.property, {
      name: 'callParent'
    });
  }
  function isExtDefineCall(extNames) {
    ...
    return function (path) {
      ...
      return t.isMemberExpression(callee) && t.isIdentifier(callee.object) && extNames.includes(callee.object.name) && t.isIdentifier(callee.property, {
        name: 'define'
      });
    };
  }
  ...
  return {
    visitor: {
      CallExpression: function CallExpression(path, state) {
        ...
        var isAsyncFunction = clsMethod && clsMethod.node.async;
